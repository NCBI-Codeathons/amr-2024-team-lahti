...
combined_summary_df %>%
  kbl(caption = "Summary of Categorical and Numeric Metadata Features", 
      col.names = c("Feature", "Summary")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = F, font_size = 20) %>%
  column_spec(1, bold = TRUE) %>%
  row_spec(0, bold = TRUE) %>%
  add_header_above(c(" " = 1, "Summary of Features" = 1)) %>%
  footnote(general = "This table summarizes sample metadata, including levels for categorical features and statistical summaries for numeric features.")
